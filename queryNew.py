...
while True:
    ...
    if option == "1":
        ...
        encrypted_file_path = input("Ingresa la ruta del archivo encriptado: ")

        # Get the encryption key from the user.
        encryption_key = input("Ingresa la clave de encriptación: ")

        # Download the encrypted file from the server and decrypt it.
        decrypted_file_data = download_file.download_file(encrypted_file_path, encryption_key)

        # Save the decrypted file data to a file.
        with open("decrypted_file.dat", "wb") as f:
            f.write(decrypted_file_data)

        # Print the decrypted file data.
        print("El archivo se ha descifrado correctamente.")
        print("Los datos del archivo descifrado son:", decrypted_file_data)
